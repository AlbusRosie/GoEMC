...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (isset($_FILES['image_main']) && $_FILES['image_main']['error'] == 0) {
        ...
        if (in_array($file_extension, $allowed_extensions)) {
            $filename = 'product_' . time() . '_main.' . $file_extension;
            $filepath = $upload_dir . $filename;
            
            if (move_uploaded_file($_FILES['image_main']['tmp_name'], $filepath)) {
                $data['image_'] = 'assets/uploads/' . $filename;
            }
        }
    } else {
        $data['image_'] = $product['image_'];
    }
    
    // Xử lý upload hình ảnh mô tả
    if (isset($_FILES['image_des']) && $_FILES['image_des']['error'] == 0) {
        $upload_dir = '../assets/uploads/';
        $file_extension = strtolower(pathinfo($_FILES['image_des']['name'], PATHINFO_EXTENSION));
        $allowed_extensions = ['jpg', 'jpeg', 'png', 'gif', 'webp'];
        
        if (in_array($file_extension, $allowed_extensions)) {
            $filename = 'product_' . time() . '_des.' . $file_extension;
            $filepath = $upload_dir . $filename;
            
            if (move_uploaded_file($_FILES['image_des']['tmp_name'], $filepath)) {
                $data['image_des'] = 'assets/uploads/' . $filename;
            }
        }
    } else {
        $data['image_des'] = $product['image_des'];
    }
    ...
    if (empty($data['name'])) {
        ...
        if ($productModel->update($product_id, $data)) {
            // Xóa tất cả detail products cũ
            $productModel->deleteAllDetails($product_id);
            
            // Thêm detail products mới
...
// Lấy detail products hiện tại
$detail_products = $productModel->getDetails($product_id);

// Lấy product options hiện tại
$current_options = $productOptionModel->getByProductId($product_id);
?>
...
<body>
        ...
        <div class="row">
            ...
            <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
                <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
                    <h1 class="h2">Sửa sản phẩm: <?php echo htmlspecialchars($product['name']); ?></h1>
                        ...
                        <form method="POST" enctype="multipart/form-data">
                            <div class="row">
                                            ...
                                            <div class="mb-3">
                                                ...
                                                <div class="input-group">
                                                    <input type="number" class="form-control" id="sale" name="sale" 
                                                           value="<?php echo $product['sale']; ?>" min="0" step="1000">
                                                    <span class="input-group-text">₫</span>
                                                </div>
                                            </div>
                                        </div>
                                                ...
                                                <div class="color-selection">
                                                    <div class="color-options d-flex flex-wrap gap-2 mb-2">
                                                        ...
                                                        <div class="color-option" data-color="Nâu đậm" data-hex="#654321" style="background-color: #654321;" title="Nâu đậm">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Nâu đậm" <?php echo in_array('Nâu đậm', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Nâu sáng" data-hex="#D2691E" style="background-color: #D2691E;" title="Nâu sáng">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Nâu sáng" <?php echo in_array('Nâu sáng', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Đen" data-hex="#000000" style="background-color: #000000;" title="Đen">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Đen" <?php echo in_array('Đen', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Trắng" data-hex="#FFFFFF" style="background-color: #FFFFFF; border: 1px solid #ddd;" title="Trắng">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Trắng" <?php echo in_array('Trắng', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Xám" data-hex="#808080" style="background-color: #808080;" title="Xám">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Xám" <?php echo in_array('Xám', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Kem" data-hex="#F5F5DC" style="background-color: #F5F5DC; border: 1px solid #ddd;" title="Kem">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Kem" <?php echo in_array('Kem', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Vàng" data-hex="#FFD700" style="background-color: #FFD700;" title="Vàng">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Vàng" <?php echo in_array('Vàng', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Đỏ" data-hex="#FF0000" style="background-color: #FF0000;" title="Đỏ">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Đỏ" <?php echo in_array('Đỏ', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Hồng" data-hex="#FFC0CB" style="background-color: #FFC0CB;" title="Hồng">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Hồng" <?php echo in_array('Hồng', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Xanh lá" data-hex="#228B22" style="background-color: #228B22;" title="Xanh lá">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Xanh lá" <?php echo in_array('Xanh lá', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Xanh dương" data-hex="#0000FF" style="background-color: #0000FF;" title="Xanh dương">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Xanh dương" <?php echo in_array('Xanh dương', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Tím" data-hex="#800080" style="background-color: #800080;" title="Tím">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Tím" <?php echo in_array('Tím', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option" data-color="Cam" data-hex="#FFA500" style="background-color: #FFA500;" title="Cam">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Cam" <?php echo in_array('Cam', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                        <div class="color-option custom-color" data-color="Khác" data-hex="" style="background: linear-gradient(45deg, #ff0000, #00ff00, #0000ff);" title="Khác (tùy chỉnh)">
                                                            <input type="checkbox" class="color-checkbox" name="colors[]" value="Khác" <?php echo in_array('Khác', $currentColors) ? 'checked' : ''; ?>>
                                                        </div>
                                                    </div>
                                    ...
                                    <div class="mb-4">
                                        ...
                                        <div id="product-options-container" class="mb-3">
                                            <?php if (!empty($current_options)): ?>
                                                <?php foreach ($current_options as $option_index => $option): ?>
                                                <div class="option-card">
                                                    <div class="option-header">
                                                        <h6 class="mb-0">Tùy chọn #<?php echo $option_index + 1; ?></h6>
                                                    ...
                                                    <div class="option-body">
                                                        ...
                                                        <div class="mb-3">
                                                            ...
                                                            <textarea class="form-control" name="option_descriptions[]" rows="2" 
                                                                      placeholder="Mô tả chi tiết về tùy chọn này..."><?php echo htmlspecialchars($option['description']); ?></textarea>
                                                        </div>
                                                        
                                                        <div class="mb-3">
                                                            <label class="form-label">Các giá trị tùy chọn</label>
                                                            <div class="values-container">
                                                                <?php if (!empty($option['values'])): ?>
                                                                    ...
                                                                    <div class="value-item">
                                                                        <div class="row">
                                                                            <div class="col-md-3">
                                                                                <input type="text" class="form-control" name="option_values[<?php echo $option_index; ?>][]" 
                                                                                       value="<?php echo htmlspecialchars($value['value']); ?>" placeholder="Giá trị" required>
                                                                            </div>
                                                                            <div class="col-md-2">
                                                                                <input type="number" class="form-control" name="option_value_stocks[<?php echo $option_index; ?>][]" 
                                                                                       value="<?php echo $value['stock_quantity']; ?>" placeholder="Tồn kho" min="0">
                                                                            </div>
                                                                            <div class="col-md-1">
                                                    ...
                                                    <div class="row">
                                                        ...
                                                        <div class="col-md-6">
                                                            <textarea class="form-control" name="detail_descriptions[]" rows="2" 
                                                                      placeholder="Mô tả chi tiết..."><?php echo htmlspecialchars($detail['description']); ?></textarea>
                                                        </div>
                                                        <div class="col-md-1">
                                ...
                                <div class="col-md-4">
                                    ...
                                    <div class="mb-3">
                                        <label for="image_main" class="form-label">Hình ảnh chính</label>
                                        <?php if($product['image_']): ?>
                                        <div class="mb-2">
                                            <p class="text-muted small">Hình ảnh hiện tại:</p>
                                            <img src="<?php echo '../' . $product['image_']; ?>" alt="Current image" class="current-image">
                                        </div>
                                        <?php endif; ?>
                                        <input type="file" class="form-control" id="image_main" name="image_main" 
                                               accept="image/*" onchange="previewImage(this, 'preview_main')">
                                        <div class="mt-2">
                                            <img id="preview_main" class="preview-image d-none" alt="Preview">
                                        </div>
                                    </div>

                                    <div class="mb-3">
                                        <label for="image_des" class="form-label">Hình ảnh mô tả</label>
                                        <?php if($product['image_des']): ?>
                                        <div class="mb-2">
                                            <p class="text-muted small">Hình ảnh hiện tại:</p>
                                            <img src="<?php echo '../' . $product['image_des']; ?>" alt="Current image" class="current-image">
                                        </div>
                                        <?php endif; ?>
                                        <input type="file" class="form-control" id="image_des" name="image_des" 
                                               accept="image/*" onchange="previewImage(this, 'preview_des')">
                                        <div class="mt-2">
                                            <img id="preview_des" class="preview-image d-none" alt="Preview">
                                        </div>
                                    </div>
                                    ...
                                    <div class="alert alert-info">
                                        ...
                                        <strong>Lưu ý:</strong>
                                        <ul class="mb-0 mt-2">
                                            <li>Để trống nếu không muốn thay đổi hình ảnh</li>
    ...
    <script>
        ...
        let optionCounter = <?php echo count($current_options); ?>;
